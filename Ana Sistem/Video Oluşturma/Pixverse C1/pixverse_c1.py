...
C1_SUPPORTED_ASPECT_RATIOS = {"16:9", "4:3", "1:1", "3:4", "9:16", "2:3", "3:2"}
...
def parse_duration(value: str) -> int:
    ...
    if not match:
        raise ConfigError(f"Süre biçimi hatalı: {value}. PixVerse C1 için 1s ile 15s arasında bir değer kullan.")

    duration = int(match.group(1))
    if duration in SUPPORTED_DURATIONS:
        return duration

    raise ConfigError(f"PixVerse C1 için süre yalnızca 1s ile 15s arasında olabilir. Gelen: {value}")
...
def normalize_video_model(value: str) -> str:
    ...
    raise ConfigError(f"Desteklenmeyen model: {value}. Bu script artık yalnızca PixVerse C1 kullanır.")
...
def read_video_settings() -> dict:
    ...
    if aspect_ratio not in C1_SUPPORTED_ASPECT_RATIOS:
        raise ConfigError("PixVerse C1 için boyut yalnızca şu oranlardan biri olabilir: 16:9, 4:3, 1:1, 3:4, 9:16, 2:3, 3:2.")
...
def validate_model_options(model: str, quality: str, duration: int, aspect_ratio: str):
    if aspect_ratio not in C1_SUPPORTED_ASPECT_RATIOS:
        raise ConfigError("PixVerse C1 için boyut yalnızca şu oranlardan biri olabilir: 16:9, 4:3, 1:1, 3:4, 9:16, 2:3, 3:2.")
    if model != "pixverse-c1":
        raise ConfigError("Model yalnızca PixVerse C1 olabilir.")
    if quality not in {"360p", "540p", "720p", "1080p"}:
        raise ConfigError("PixVerse C1 için quality yalnızca 360p, 540p, 720p veya 1080p olabilir.")
    if duration not in SUPPORTED_DURATIONS:
        raise ConfigError("PixVerse C1 için duration yalnızca 1 ile 15 saniye arasında olabilir.")
...
def pixverse_c1_toplu(auto_download: bool = True, force_relogin: bool = False):
    ...
    print(f"  Ses: {settings['ses']} (--audio bayrağı ile aktif edilir)")
...
def main():
    parser = argparse.ArgumentParser(description="PixVerse C1 - app.py ile entegre toplu sürüm")
    ...
    try:
        ...
        pixverse_c1_toplu(auto_download=not args.no_download, force_relogin=args.force_relogin)
    except SystemExit:
        raise
