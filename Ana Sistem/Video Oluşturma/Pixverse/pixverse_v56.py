...
SUPPORTED_DURATIONS = set(range(1, 16))
V6_SUPPORTED_ASPECT_RATIOS = {"16:9", "4:3", "1:1", "3:4", "9:16", "2:3", "3:2", "21:9"}
DEFAULT_MODEL = "v6"
...
def parse_duration(value: str) -> int:
    ...
    match = re.fullmatch(r"(\d+)(?:s|sn|sec|secs|second|seconds)?", normalized)
    if not match:
        raise ConfigError(f"Süre biçimi hatalı: {value}. V6 için 1s ile 15s arasında bir değer kullan.")

    duration = int(match.group(1))
    if duration in SUPPORTED_DURATIONS:
        return duration

    raise ConfigError(f"V6 için süre yalnızca 1s ile 15s arasında olabilir. Gelen: {value}")
...
def normalize_video_model(value: str) -> str:
    ...
    if compact in {"v6", "pixversev6", "pixverse6"}:
        return "v6"

    if compact in {"v5.6", "v56", "pixversev5.6", "pixversev56", "pixverse5.6", "pixverse56"}:
        print("[INFO] Ayarlarda eski V5.6 model değeri bulundu; otomatik olarak V6 kullanılacak.")
        return "v6"

    raise ConfigError(f"Desteklenmeyen model: {value}. Bu script artık yalnızca PixVerse V6 kullanır.")


def normalize_quality(value: str) -> str:
    normalized = str(value or "").strip().lower()
    if normalized in {"360", "540", "720", "1080"}:
...
def read_video_settings() -> dict:
    ...
    quality = normalize_quality(lines[3]) if len(lines) > 3 else DEFAULT_QUALITY
    model = normalize_video_model(lines[4] if len(lines) > 4 else DEFAULT_MODEL)

    if aspect_ratio not in V6_SUPPORTED_ASPECT_RATIOS:
        raise ConfigError("V6 için boyut yalnızca şu oranlardan biri olabilir: 16:9, 4:3, 1:1, 3:4, 9:16, 2:3, 3:2, 21:9.")
...
def validate_model_options(model: str, quality: str, duration: int, aspect_ratio: str):
    if aspect_ratio not in V6_SUPPORTED_ASPECT_RATIOS:
        raise ConfigError("V6 için boyut yalnızca şu oranlardan biri olabilir: 16:9, 4:3, 1:1, 3:4, 9:16, 2:3, 3:2, 21:9.")
    if model != "v6":
        raise ConfigError("Model yalnızca v6 olabilir.")
    if quality not in {"360p", "540p", "720p", "1080p"}:
        raise ConfigError("V6 için quality yalnızca 360p, 540p, 720p veya 1080p olabilir.")
    if duration not in SUPPORTED_DURATIONS:
        raise ConfigError("V6 için duration yalnızca 1 ile 15 saniye arasında olabilir.")
...
def generate_video_for_prompt(
    ...
    audio_enabled = settings.get("ses", "kapali").lower() in ("acik", "açik", "evet", "yes", "true", "on")
    print(f"Ses ayari: {'acik' if audio_enabled else 'kapali'}")
    ...
    for index, varyant in enumerate(prompt_varyantlari, start=1):
        ...
        if generation_mode == "transition":
            print(
                ...
                f"transition=start+end, audio={'acik' if audio_enabled else 'kapali'}"
            )
...
def pixverse_v56_toplu(auto_download: bool = True, force_relogin: bool = False):
    ...
    print(f"  Ses: {settings['ses']} (V6 için --audio bayrağı ile aktif edilir)")
...
def main():
    parser = argparse.ArgumentParser(description="PixVerse V6 - app.py ile entegre toplu sürüm")
    ...
    try:
        ...
        pixverse_v56_toplu(auto_download=not args.no_download, force_relogin=args.force_relogin)
    except SystemExit:
        raise
