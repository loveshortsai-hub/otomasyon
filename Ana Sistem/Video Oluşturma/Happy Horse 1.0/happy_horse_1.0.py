...
def generate_video_for_prompt(
    ...
    if generation_mode == "transition":
        raise UserFacingError(
            status="Uretim modu",
            reason=f"{MODEL_LABEL} Transition modunu desteklemiyor",
            detail="PixVerse CLI 1.1.6 destek matrisinde Happy Horse 1.0 yalnizca create video icin listeleniyor.",
            result="Video Basarisiz",
        )
...
def happy_horse_10_toplu(auto_download: bool = True, force_relogin: bool = False):
    ...
    print(f"  Ses: {settings['ses']} (Happy Horse 1.0 CLI'da uygulanacak)")
...
def main():
    parser = argparse.ArgumentParser(description="Happy Horse 1.0 - app.py ile entegre toplu sürüm")
    ...
    try:
        ...
        happy_horse_10_toplu(auto_download=not args.no_download, force_relogin=args.force_relogin)
    except SystemExit:
        raise
