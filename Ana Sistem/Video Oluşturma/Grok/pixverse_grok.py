...
def parse_duration(value: str) -> int:
    ...
    if not match:
        raise ConfigError(f"Süre biçimi hatalı: {value}. Yalnızca 5s, 8s, 10s, 15s kullan.")
    ...
    if duration == 4:
        print("[WARN] 4s, Grok Imagine tarafından desteklenmiyor. 5s kullanılacak.")
        return 5
    if duration == 12:
        print("[WARN] 12s, Grok Imagine tarafından desteklenmiyor. 10s kullanılacak.")
        return 10

    raise ConfigError(f"Süre yalnızca 5s, 8s, 10s, 15s olabilir. Gelen: {value}")
...
def validate_model_options(model: str, quality: str, duration: int, aspect_ratio: str):
    ...
    if model != "grok-imagine":
        raise ConfigError("Model yalnızca grok-imagine olabilir.")
    if quality not in {"360p", "540p", "720p"}:
        raise ConfigError("Grok Imagine için quality yalnızca 360p, 540p veya 720p olabilir.")
    if duration not in SUPPORTED_DURATIONS:
        raise ConfigError("Grok Imagine için duration yalnızca 5, 8, 10, 15 olabilir.")
...
def pixverse_grok_toplu(auto_download: bool = True, force_relogin: bool = False):
...
def main():
    parser = argparse.ArgumentParser(description="PixVerse Grok Imagine - app.py ile entegre toplu sürüm")
    ...
    try:
        ...
        pixverse_grok_toplu(auto_download=not args.no_download, force_relogin=args.force_relogin)
    except SystemExit:
        raise
