...
def prompt_oku(prompt_klasor_adi: str) -> str:
    try:
        ...
        prompt_metni = read_text_file(prompt_dosya_yolu).strip()
        if not prompt_metni:
            raise ConfigError("prompt.txt boş")

        print(f"✓ Prompt okundu: {len(prompt_metni)} karakter (tam prompt)")
        return prompt_metni
    except Exception as e:
...
def parse_duration(value: str) -> int:
    ...
    if not match:
        raise ConfigError(f"Süre biçimi hatalı: {value}. Yalnızca 4s, 8s, 12s kullan.")
    ...
    if duration == 5:
        print("[WARN] 5s, Sora 2 tarafından desteklenmiyor. 4s kullanılacak.")
        return 4

    raise ConfigError(f"Süre yalnızca 4s, 8s, 12s olabilir. Gelen: {value}")
...
def validate_model_options(model: str, quality: str, duration: int, aspect_ratio: str):
    ...
    if model == "sora-2":
        if quality != "720p":
            raise ConfigError("sora-2 için quality yalnızca 720p olabilir.")
        if duration not in SUPPORTED_DURATIONS:
            raise ConfigError("sora-2 için duration yalnızca 4, 8, 12 olabilir.")
    elif model == "sora-2-pro":
        if quality not in {"720p", "1080p"}:
            raise ConfigError("sora-2-pro için quality yalnızca 720p veya 1080p olabilir.")
        if duration not in SUPPORTED_DURATIONS:
            raise ConfigError("sora-2-pro için duration yalnızca 4, 8, 12 olabilir.")
    else:
        raise ConfigError("Model yalnızca sora-2 veya sora-2-pro olabilir.")
...
def build_user_error(stage: str, stdout: str = "", stderr: str = "", return_code: Optional[int] = None, elapsed: Optional[int] = None):
    ...
    if error_code == 400018 or "cannot exceed 2048 characters" in haystack or "prompt too long" in haystack:
        return UserFacingError(
            status=stage,
            reason="Prompt çok uzun",
            detail="2048 karakter sınırı aşıldı.",
            result="Video Başarısız",
        )
...
def pixverse_sora2_toplu(auto_download: bool = True, force_relogin: bool = False):
    ...
    print(f"  Ses: {settings['ses']} (Sora 2 CLI için bilgi amaçlı)")
...
def main():
    parser = argparse.ArgumentParser(description="PixVerse Sora 2 - app.py ile entegre toplu sürüm")
    ...
    try:
        ...
        pixverse_sora2_toplu(auto_download=not args.no_download, force_relogin=args.force_relogin)
    except SystemExit:
        raise
