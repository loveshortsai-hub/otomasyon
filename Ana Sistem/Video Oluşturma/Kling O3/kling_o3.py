...
def build_user_error(stage: str, stdout: str = "", stderr: str = "", return_code: Optional[int] = None, elapsed: Optional[int] = None):
    ...
    invalid_param_detected = (
        "invalid param" in haystack
        or "400017" in haystack and "param" in haystack
    )
    if invalid_param_detected:
        return UserFacingError(
            status=stage,
            reason="Model parametreleri reddedildi",
            detail="Video modeli gonderilen parametre kombinasyonunu kabul etmedi. Wrapper artik ses ayarina gore --audio veya --no-audio ve uyumluluk icin --no-multi-shot gonderir; sorun devam ederse modelin create video modu hesabinizda veya workspace'inizde henuz aktif olmayabilir.",
            duration=elapsed,
    ...
    if str(generic_detail).strip() in {"{}", "{"}:
        generic_detail = "CLI boş hata yanıtı döndürdü. Gönderilen bayraklar model tarafından desteklenmiyor olabilir."
    return UserFacingError(
        status=stage,
        reason="İşlem tamamlanamadı",
...
def generate_video_for_prompt(prompt_klasor_adi: str, prompt: str, settings: dict, reference_image: Optional[Path], output_dir: Path, auto_download: bool = True) -> dict:
    ...
    print(
        ...
        f"image={'var' if reference_image else 'yok'}"
    )
...
def kling_o3_toplu(auto_download: bool = True, force_relogin: bool = False):
    ...
    print(f"  Ses: {settings['ses']} (Kling O3 CLI'da uygulanacak)")
...
def main():
    parser = argparse.ArgumentParser(description="Kling O3 - app.py ile entegre toplu sürüm")
    ...
    try:
        ...
        kling_o3_toplu(auto_download=not args.no_download, force_relogin=args.force_relogin)
    except SystemExit:
        raise
